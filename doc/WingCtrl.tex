\chapter{Aktywne t³umienie flatteru skrzyd³a}
Celem poni¿szego rozdzia³u jest opracowanie systemu aktywnego t³umienia flatteru skrzyd³a, który pos³u¿y za punkt odniesienia dla dalszej czêœci pracy. Kryteria osi¹gowe projektowanego systemu zosta³y opracowane przy uwzglêdnieniu zmiennych parametrów pracy, oraz jego umiejscowienia w ca³ym uk³adzie sterowania. Nale¿y bowiem pamiêtaæ, ¿e system t³umienia flatteru stanowi wewnêtrzn¹ pêtlê sprzê¿enia zwrotnego (rys.\ref{RysSchematWingCtrl}). Na zewn¹trz jest zawsze cz³owiek, lub system sterowania lotem (autopilot), który wykorzystuje ten sam kana³ sterowania.

\begin{figure}[hbt]
\centering
\includegraphics[width=\linewidth/2]{SchematWingCtrl.png}
\caption{Umiejscowienie systemu st³umienia flatteru w ca³ym uk³adzie.}
\label{RysSchematWingCtrl}
\end{figure}

\section{Warunki pracy}
Podstawowym parametrem wp³ywaj¹cym na dynamikê obiektu jest ciœnienie dynamiczne~q. Zakres zmiennoœci tego parametru wynika z osi¹gów samolotu - zakresu prêdkoœci i wysokoœci lotu. Prêdkoœæ maksymalna zosta³a arbitralnie za³o¿ona na potrzeby projektu i znacznie przekracza prêdkoœæ krytyczn¹ flatteru dla uk³adu otwartego.

Drugim wa¿nym czynnikiem jest poziom paliwa, który ma poœredni wp³yw na wiele parametrów takich jak:\\*
- po³o¿enie œrodka ciê¿koœci (które jest krytyczne w zagadnieniach flatteru)\\*
...
Poniewa¿ ze wzrostem iloœci paliwa w skrzydle, po³o¿enie œrodka ciê¿koœci przesuwa siê do ty³u, prêdkoœæ krytyczna flatteru maleje, co mo¿na zaobserwowaæ na rys.~\ref{RysPredkosci}.

\section{Za³o¿enia projektowe}
Podstawowym celem aktywnego systemu t³umienia flatteru jest zwiêkszenie prêdkoœci krytycznej flatteru \( V_k \), {\bf przy uwzglêdnieniu ograniczeñ uk³adu wykonawczego}.

Definicja prêdkoœci krytycznej jest œciœle zwi¹zana ze stabilnoœci¹, ale sama stabilnoœæ nie jest kryterium wystarczaj¹cym. Nale¿a³oby zaostrzyæ tê definicjê, poniewa¿ nawet w zakresie stabilnej pracy mo¿e dojœæ do przekroczenia dopuszczalnych naprê¿eñ konstrukcji. 

Zgodnie z kryteriami konstrukcyjnymi {\bf JAR-23} \cite{JAR23}, dla samolotu Kategorii Normalnej obowi¹zuje wspó³czynnik obci¹¿eñ dopuszczalnych w locie n = 3,8. Maksymalne wartoœci zniekszta³ceñ p³ata wynikaj¹ nie z modelu samego skrzyd³a, ale z modelu ca³ego samolotu. Podczas manewru ze wspó³czynnikiem obci¹¿eñ n = 3,8 i maksymaln¹ mas¹ startow¹, odkszta³cenia p³ata wynios¹ odpowiednio:
...
\begin{itemize}
\item wysokoœæ lotu \( h \in \left[0 ... 11000 \right] \) m.
...
\begin{equation}
K_2 = \sqrt{\frac{1}{T}}\int{\left[\delta_c(t) \right]^2} dt
\label{EqWysilekSterowania}
\end{equation}

Trzecie kryterium dotyczy wysi³ku sterowania ze wzglêdu na zewnêtrzny sygna³ steruj¹cy \( \delta_p(t) \) w postaci skoku jednostkowego o wartoœci 70\% maksymalnego wychylenia powierzchni sterowej \( ( \delta_p = 0,7 \cdot \delta_{max} ) \). Wartoœæ \( K_3 \) jest liczona analogicznie do \( K_2 \).

(TODO) Tabelka z kryteriami?
...
Standardowo u¿ywanymi przyrz¹dami pomiarowymi w praktycznych systemach t³umienia flatteru s¹ przyspieszeniomierze \cite{Vivek}. Odpowiednie kombinacje liniowe sygna³ów z dwóch przyspieszeniomierzy umieszczonych na krawêdziach skrzyd³a (na wysokoœci profilu odniesienia) odpowiadaj¹ \( \ddot{h}(t) \) i \( \ddot{\theta}(t) \).
...
Filtr washout dobrano tak, aby mia³ minimalny wp³yw na sygna³ pomiarowy w okolicach czêstotliwoœci flatteru \( f_f = 6,67 Hz \). Czêstotliwoœæ kompensatora jest o rz¹d wielkoœci mniejsza od \( f_f \).
...
\section{Oszacowanie fizycznych mo¿liwoœci}
Synteza regulatora, a w szczególnoœci wynikowa wartoœæ normy wa¿onego uk³adu zamkniêtego \( \| T_{zw} \|_\infty \), pozwalaj¹ wyznaczyæ fizyczne ograniczenia uk³adu wykonawczego. Model liniowy nie dostarcza tych informacji, odpowiednio du¿e wzmocnienie jest w stanie zapewniæ stabilnoœæ dowolnego uk³adu liniowego.

Dokonuj¹c syntezy regulatora dla coraz wiêkszej prêdkoœci, otrzymamy zapewne graniczn¹ wartoœæ prêdkoœci, powy¿ej której spe³nienie za³o¿eñ projektowych staje siê niemo¿liwe.

TODO: Wykres \( \gamma \) wzglêdem prêdkoœci.
TODO: Przyk³adowe przebiegi czasowe dla granicznej prêdkoœci.
...
TODO: Poziom odpornoœci uk³adu zamkniêtego (\(V_{fcl}/V_{fol}\)) mocno uzale¿niony od doboru Gn.

\subsection{Hinf 3}
W kolejnym podejœciu postarano siê uwzglêdniæ model niepewnoœci wyprowadzony w poprzednim rozdziale, w celu zwiêkszenia obszaru stabilnej pracy uk³adu zamkniêtego.

Aby uwzglêdniæ model niepewnoœci, nale¿y tak przekonfigurowaæ generyczny model \( P \), aby wynikowa macierz transmitancji \( T_{zw}(s) \) zawiera³a odpowiedni cz³on. W przypadku niepewnoœci wzglêdnej na wyjœciu jest to cz³on \(G(s)K(s)(I-G(s)K(s))^{-1}\). Schemat blokowy przedstawia rys. (\ref{WingModelP2}).

\begin{figure}
\centering
\includegraphics[width=\linewidth/2]{WingModelP2.png}
\caption{Uogólniony model \( P \)}
\label{WingModelP2}
\end{figure}

Niestety, podobnie jak w \cite{Waszak}, model okaza³ siê zbyt obszerny. Warunek odpornej stabilnoœci zarówno dla wzglêdnego jak i bezwzglêdnego modelu niepewnoœci jest zbyt zachowawczy. Uzyskane \(\gamma >> 1\) oznacza, ¿e wymagania s¹ nieosi¹galne.

Próba dalszego rozszerzenia zakresu stabilnej pracy uk³adu mog³aby byæ rozwi¹zana poprzez wprowadzenie regulatora parametryzowanego prêdkoœci¹ lotu (gain scheduling).

\begin{figure}
\centering
\includegraphics[width=\linewidth]{WingCtrlKryt1.eps}
\caption{Porównanie kryterium "wysi³ek sterowania wzglêdem sygna³u od pilota" dla ró¿nych prêdkoœci. Pionowa czarna linia przerywana - prêdkoœæ flatteru uk³adu otwartego. Pionowa czerwona linia - prêdkoœæ flatteru uk³adu zamkniêtego }
\end{figure}
