...
Cztery sta³e bloki na schemacie \ref{SchematModelStr} mog¹ zostaæ zast¹pione macierzami uwzglêdniaj¹cymi \( \delta_l \), tak, jak to pokazano na rys. \ref{SchematModelStrU}. 
\begin{figure}[hbt]
\centering
...
\end{matrix}\right]
\end{equation*}
Otrzymaliœmy w ten sposób nowe (,,sztuczne'') sygna³y wejœciowe i wyjœciowe - odpowiednio \( r = [r_m, r_I, r_{s_1}, r_{s_2}] \) i \( z = [z_m, z_I, z_{s_1}, z_{s_2} ] \). Model strukturalny uwzglêdniaj¹cy zmienne parametry, przedstawiono na rys.~\ref{SchematModelDelta}. Nale¿y zaznaczyæ, ¿e macierz \( \Delta \) sk³ada siê wy³¹cznie z powtórzonej wartoœci \( \delta_l \). Wyznaczaj¹c \( G_p(s) = L_u(G_\Delta(s), \Delta) \) otrzymujemy model strukturalny skrzyd³a dla pewnej wartoœci poziomu paliwa.
...
\caption{Schemat blokowy modelu z uwzglêdnieniem niepewnoœci parametrycznej}
\label{SchematModelDelta}
\end{figure}



Ostateczna postaæ macierzy modelu:
...
Wykorzystuj¹c powy¿sz¹ metodologiê, z pomoc¹ narzêdzi {\it Robust Control Toolbox } wygenerowano model z ustrukturyzowan¹ niepewnoœci¹ parametryczn¹ uwzglêdniaj¹c¹ wszystkie trzy niepewne parametry (poziom paliwa, prêdkoœæ i wysokoœæ lotu).

\subsection{Analiza strukturalnej wartoœci osobliwej}
Analiza strukturalnej wartoœci osobliwej pozwala obliczyæ granice stabilnoœci uk³adu. Dla uproszczenia analizy, ograniczono siê do prêdkoœci jako niepewnego parametru. Wyznaczenie strukturalnej wartoœci osobliwej uk³adu \( G_p(s) \) wzglêdem znormalizowanej macierzy niepewnoœci \( \Delta \) wymaga oszacowania wartoœci \( \mu_\Delta(G(j\omega) \) dla ka¿dej \( \omega \in \mathcal{R} \). Oczywiœcie konieczne jest wybranie pewnego dyskretnego zbioru \( \omega_i \), który bêdzie odpowiedni dla danego obiektu. Zbiór ten musi uwzglêdniaæ zakres pulsacji, w którym spodziewamy siê wyst¹pienia niestabilnoœci - w tym przypadku bêdzie to czêstotliwoœæ flatteru. W ogólnoœci najpierw nale¿y przeanalizowaæ wartoœæ \( \mu_\Delta \) dla mo¿liwie du¿ego ale rzadkiego zbioru \( \omega_i \), a nastêpnie zawêziæ poszukiwania do zakresu, gdzie górne oszacowanie \( \mu_\Delta \) przyjmuje najwiêksze wartoœci.

Je¿eli znamy czêstotliwoœæ niestabilnej postaci ruchu, najprostszym sposobem znalezienia \( \mu_\Delta(G) \) jest poszukiwanie maksimum lokalnego w okolicach tej czêstotliwoœci.

\begin{figure}[hbt]
\centering
\includegraphics[width=\linewidth]{NiepewnoscUrojona.eps}
\caption{Porównanie oszacowania strukturalnej wartoœci osobliwej wzglêdem niepewnoœci czysto rzeczywistej oraz zawieraj¹cej 1\% czêœci urojonej}
\label{NiepewnoscUrojona}
\end{figure}

Problemem zwi¹zanym z wykorzystaniem strukturalnej wartoœci osobliwej jest to, ¿e w modelach z parametryczn¹ niepewnoœci¹, gdy macierz \( \Delta \) sk³ada siê z liczb rzeczywistych, zbie¿noœæ algorytmu oszacowania \( \mu_\Delta \) jest niezadowalaj¹ca, a czêsto w ogóle niemo¿liwe jest oszacowanie dolnej wartoœci \( \mu_\Delta \). Praktycznym sposobem radzenia sobie z tym problemem jest sztuczne wprowadzenie urojonej niepewnoœci \cite{RobustMatlab}. W tej sytuacji macierz \( \Delta \) rozszerzamy o dodatkowe elementy, które mog¹ przyjmowaæ wartoœci ze zbioru liczb zespolonych.
\[
\Delta = \left[ 
\begin{matrix} 
\begin{matrix} 
...
\]
Zak³adaj¹c, ¿e chcemy wprowadziæ 1\% udzia³ wartoœci urojonej, przekszta³camy analizowan¹ macierz \( M \):
\begin{gather*}
...
Wystêpowanie liczby zespolonej w macierzy niepewnoœci nie ma fizycznego sensu (oznacza³oby to np. przesuniêcie fazy przyspieszenia wzglêdem si³y po podzieleniu przez masê). Z praktycznych wzglêdów jest to jednak uzasadnione, poniewa¿ nie wprowadza znacz¹cych przek³amañ (rys.~\ref{ModelUrojonaNiep}).

\begin{figure}[hbt]
\centering
\includegraphics[width=\linewidth]{ModelUrojonaNiep.eps}
\caption{Porównanie charakterystyki czêstotliwoœciowej modeli z rzeczywist¹ i urojon¹ wartoœci¹ zaburzenia \( \delta_{U} \)}
\label{ModelUrojonaNiep}
\end{figure}

Najwa¿niejsze z punktu widzenia analizy stabilnoœci jest górne oszacowanie \( \bar{\mu_\Delta} (G(s)) \). Na rys.~\ref{PikMu} obserwujemy lokalne maksimum odpowiadaj¹ce pulsacji flatteru (44,5 rad/s). Samo przekroczenie wartoœci 1 oznacza brak odpornej stabilnoœci. Dodatkowo na podstawie odczytanej wartoœci mo¿emy okreœliæ graniczn¹ wartoœæ prêdkoœci, dla której uk³ad jest stabilny:
\begin{gather}
...
\caption{Gorne oszacowanie wartoœci \( \mu_\Delta(G(s)) \). Widoczne lokalne maksimum dla czêstotliwoœci flatteru.}
\label{PikMu}
\end{figure}
